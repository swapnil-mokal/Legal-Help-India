    ...
    switch (d._fn) {
      ...
      case "admin-legal-requests":
        return out_(admin_(d, function () {
          if (d._method === "POST") return d.action === "note" ? setNote_(d) : setStatus_(d);
          return { requests: read_(LEGAL_SHEET, LEGAL_COLS, LEGAL_HEADERS).sort(newest_) };
        }));
      case "track-request": return out_(trackRequest_(d));
      case "admin-settings": return out_(admin_(d, function () { return { settings: getSettings_() }; }));
      case "public-settings": return out_({settings:getSettings_()});
      case "save-settings": return out_(admin_(d, function () { return saveSettings_(d); }));
    }
...
function submitLegal_(d) {
  ...
  // Email notification to the configured legal-services inbox.
...
function setStatus_(d) {
  if (!d.id || ["New", "Contacted", "In Progress", "Resolved"].indexOf(d.status) === -1) return { error: "invalid id or status", _status: 400 };
...
// ---------- Tracking / Notes / Content ----------
const SETTINGS_SHEET = "Settings";
const SETTINGS_HEADERS = ["Key", "Value", "Updated At"];
function getSettings_() {
  const sh = sheet_(SETTINGS_SHEET, SETTINGS_HEADERS);
  const last = sh.getLastRow();
  const out = {};
  if (last < 2) return out;
  sh.getRange(2,1,last-1,3).getValues().forEach(function(row){ if(row[0]) out[String(row[0])] = String(row[1] || ""); });
  return out;
}
function saveSettings_(d) {
  const sh = sheet_(SETTINGS_SHEET, SETTINGS_HEADERS);
  const current = getSettings_();
  ["welcome","update","cta"].forEach(function(k){ if (d[k] !== undefined) current[k] = str_(d[k], 1000); });
  if (sh.getLastRow() >= 2) sh.getRange(2,1,sh.getLastRow()-1,3).clearContent();
  Object.keys(current).forEach(function(k){ sh.appendRow([safe_(k), safe_(current[k]), new Date().toISOString()]); });
  return {ok:true, settings:current};
}
function trackRequest_(d) {
  const id = str_(d.id, 80), mobile = (d.mobile || "").toString().replace(/\D/g, "");
  if (!id || mobile.length !== 10) return {error:"Request ID आणि 10 अंकी mobile आवश्यक आहे.", _status:400};
  const rows = read_(LEGAL_SHEET, LEGAL_COLS, LEGAL_HEADERS).filter(function(r){ return r.id === id && r.mobile === mobile; });
  if (!rows.length) return {error:"विनंती सापडली नाही. Request ID आणि mobile तपासा.", _status:404};
  const r = rows[0]; return {ok:true, request:{id:r.id,status:r.status,submittedAt:r.submittedAt,category:r.category}};
}
function setNote_(d) {
  if (!d.id) return {error:"Request ID आवश्यक आहे.", _status:400};
  const sh = sheet_(LEGAL_SHEET, LEGAL_HEADERS); const row = rowOf_(sh, LEGAL_COLS, d.id);
  if (row < 0) return {error:"विनंती सापडली नाही.", _status:404};
  sh.getRange(row, LEGAL_COLS.indexOf("notes") + 1).setValue(safe_(str_(d.notes, 2000)));
  return {ok:true};
}
